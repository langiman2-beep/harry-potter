...
async function loadStudents() {
    // Находим твой родной контейнер студентов по ID
    const container = document.getElementById('students-container');
    if (!container)
        return;
    // Вызываем общую функцию и передаем ей правило фильтрации строго для студентов
    const students = await fetchData(container, char => char.hogwartsStudent === true);
    if (students.length === 0)
        return;
    // Рендерим твои оригинальные карточки
    renderStudents(students, container);
}
function renderStudents(characters, container) {
